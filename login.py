...
def open_register_window():
    ...
    label_register_username = tk.Label(register_window, text="Nome de usuário:")
    label_register_username.pack(pady=(20, 0))

    entry_register_username = tk.Entry(register_window)
    entry_register_username.pack(pady=(0, 10))

    label_register_password = tk.Label(register_window, text="Senha:")
    label_register_password.pack(pady=(10, 0))

    entry_register_password = tk.Entry(register_window, show="*")
    entry_register_password.pack(pady=(0, 20))
...
def open_admin_login_window():
    ...
    tk.Label(admin_window, text="Nome de usuário:").pack(pady=(20, 0))
    admin_username_entry = tk.Entry(admin_window)
    admin_username_entry.pack(pady=(0, 10))

    tk.Label(admin_window, text="Senha:").pack(pady=(10, 0))
    admin_password_entry = tk.Entry(admin_window, show="*")
    admin_password_entry.pack(pady=(0, 20))
...
def open_admin_approval_window():
    ...
    if not os.path.exists(USERS_FILE):
        tk.Label(admin_window, text="Nenhum usuário registrado.").pack()
        return
    ...
    with open(USERS_FILE, "r") as file:
        for line in file:
            ...
            if approved == "no":
                ...
                tk.Label(frame, text=f"Usuário: {username}").pack(side=tk.LEFT)
...
button_login = ttk.Button(root, text="Login", command=login, style="Custom.TButton")
button_login.pack(pady=(10, 20))

button_register = ttk.Button(root, text="Registrar", command=open_register_window, style="Custom.TButton")
button_register.pack(pady=(10, 20))

button_admin = ttk.Button(root, text="Administração", command=open_admin_login_window, style="Custom.TButton")
button_admin.pack(pady=(10, 20))
